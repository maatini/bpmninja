...
async fn upload_instance_file(
    ...
    let url = format!("{}/api/instances/{}/files/{}", *state.base_url.lock().unwrap(), instance_id, var_name);
    
    let file_data = tokio::fs::read(&file_path).await.map_err(|e| format!("Cannot read file: {}", e))?;
...
async fn download_instance_file(
    ...
    tokio::fs::write(&save_path, bytes).await.map_err(|e| format!("Cannot write file: {}", e))?;
...
async fn delete_instance_file(
    ...
    if !res.status().is_success() {
        return Err(format!("Delete file failed: {}", res.status()));
    }
