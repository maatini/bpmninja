...
impl WorkflowEngine {
    pub(crate) async fn handle_call_activity(
        ...
        inst.push_audit_log(format!("🔗 Call Activity '{current_id}' invoking '{called_element}'"));
        tracing::info!("Instance {instance_id}: '{current_id}' invoking '{called_element}'");
        Ok(NextAction::WaitForCallActivity { called_element: called_element.to_string(), token: token.clone() })
    }
