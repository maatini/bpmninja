...
fn linear_definition(id: &str, n_tasks: usize) -> ProcessDefinition {
    ...
    for i in 1..=n_tasks {
        ...
        builder = builder
            .node(&node_id, BpmnElement::ServiceTask { topic: format!("topic_{i}") })
...
fn parallel_definition(id: &str, n_branches: usize) -> ProcessDefinition {
    ...
    for i in 1..=n_branches {
        ...
        builder = builder
            .node(&node_id, BpmnElement::ServiceTask { topic: "parallel_task".to_string() })
            .flow("fork", &node_id)
            .flow(&node_id, "join");
    }
...
async fn drain_service_tasks(engine: &mut WorkflowEngine, worker: &str) {
    loop {
        ...
        let tasks = engine.fetch_and_lock_service_tasks(worker, 1000, &topics, 30_000).await;
        if tasks.is_empty() { break; }
        for task in tasks {
            engine.complete_service_task(task.id, worker, std::collections::HashMap::new()).await.unwrap();
        }
    }
}
// ---------------------------------------------------------------------------
...
async fn stress_memory_10000_instances() {
    ...
    for _ in 0..10_000 {
        ...
        engine.start_instance_with_variables(def_key, vars).await.unwrap();
    }
    
    let stats = engine.get_stats().await;
    assert_eq!(stats.instances_running + stats.instances_waiting_user, 10_000);
...
async fn correctness_nested_parallel_3_levels() {
    ...
    let def = ProcessDefinitionBuilder::new("nested_par")
        ...
        .node("t2a", BpmnElement::ServiceTask{topic: "t2a".into()})
        .node("t2b", BpmnElement::ServiceTask{topic: "t2b".into()})
        .node("join2", BpmnElement::ParallelGateway)
        .node("t_b", BpmnElement::ServiceTask{topic: "t_b".into()})
    ...
    for topic in topics {
         ...
         let locked = engine.fetch_and_lock_service_tasks("w", 1, &[topic.into()], 1000).await;
         engine.complete_service_task(locked[0].id, "w", vars).await.unwrap();
    }
...
async fn persistence_crash_recovery_user_task() {
    ...
    let state = persistence.list_instances().await.unwrap().into_iter().find(|i| i.id == inst_id).unwrap();
    new_engine.definitions.insert(def_key, std::sync::Arc::new(def)).await;
    ...
    for task in &tasks {
        new_engine.pending_user_tasks.insert(task.task_id, task.clone());
    }
    assert_eq!(tasks.len(), 1);
    
    new_engine.complete_user_task(tasks[0].task_id, std::collections::HashMap::new()).await.unwrap();
    assert_eq!(new_engine.get_instance_state(inst_id).await.unwrap(), InstanceState::Completed);
}
...
async fn persistence_crash_recovery_parallel() {
    ...
    // Complete T1
    engine.complete_user_task(tasks[0].task_id, std::collections::HashMap::new()).await.unwrap();
    ...
    new_engine.complete_user_task(tasks2[0].task_id, std::collections::HashMap::new()).await.unwrap();
    assert_eq!(new_engine.get_instance_state(inst_id).await.unwrap(), InstanceState::Completed);
}
...
async fn persistence_error_counter_increments() {
    ...
    impl crate::persistence::WorkflowPersistence for FailingPersistence {
        async fn save_token(&self, _: &crate::model::Token) -> EngineResult<()> { Ok(()) }
        async fn load_tokens(&self, _: &str) -> EngineResult<Vec<crate::model::Token>> { Ok(vec![]) }
        async fn save_instance(&self, _: &ProcessInstance) -> EngineResult<()> {
            Err(crate::error::EngineError::PersistenceError("Injected failure".into()))
        }
        async fn list_instances(&self) -> EngineResult<Vec<ProcessInstance>> { Ok(vec![]) }
        async fn delete_instance(&self, _: &str) -> EngineResult<()> { Ok(()) }
        async fn save_definition(&self, _: &crate::model::ProcessDefinition) -> EngineResult<()> { Ok(()) }
        async fn list_definitions(&self) -> EngineResult<Vec<crate::model::ProcessDefinition>> { Ok(vec![]) }
        async fn delete_definition(&self, _: &str) -> EngineResult<()> { Ok(()) }
        async fn save_user_task(&self, _: &crate::engine::PendingUserTask) -> EngineResult<()> { Ok(()) }
        async fn delete_user_task(&self, _: uuid::Uuid) -> EngineResult<()> { Ok(()) }
        async fn list_user_tasks(&self) -> EngineResult<Vec<crate::engine::PendingUserTask>> { Ok(vec![]) }
        async fn save_service_task(&self, _: &crate::engine::PendingServiceTask) -> EngineResult<()> { Ok(()) }
        async fn delete_service_task(&self, _: uuid::Uuid) -> EngineResult<()> { Ok(()) }
        async fn list_service_tasks(&self) -> EngineResult<Vec<crate::engine::PendingServiceTask>> { Ok(vec![]) }
        async fn save_timer(&self, _: &crate::engine::PendingTimer) -> EngineResult<()> { Ok(()) }
        async fn delete_timer(&self, _: uuid::Uuid) -> EngineResult<()> { Ok(()) }
        async fn list_timers(&self) -> EngineResult<Vec<crate::engine::PendingTimer>> { Ok(vec![]) }
        async fn save_message_catch(&self, _: &crate::engine::PendingMessageCatch) -> EngineResult<()> { Ok(()) }
        async fn delete_message_catch(&self, _: uuid::Uuid) -> EngineResult<()> { Ok(()) }
        async fn list_message_catches(&self) -> EngineResult<Vec<crate::engine::PendingMessageCatch>> { Ok(vec![]) }
        async fn save_file(&self, _: &str, _: &[u8]) -> EngineResult<()> { Ok(()) }
        async fn load_file(&self, _: &str) -> EngineResult<Vec<u8>> { Ok(vec![]) }
        async fn delete_file(&self, _: &str) -> EngineResult<()> { Ok(()) }
        async fn save_bpmn_xml(&self, _: &str, _: &str) -> EngineResult<()> { Ok(()) }
        async fn load_bpmn_xml(&self, _: &str) -> EngineResult<String> { Ok("".into()) }
        async fn list_bpmn_xml_ids(&self) -> EngineResult<Vec<String>> { Ok(vec![]) }
...
async fn edge_rhai_infinite_loop() {
    ...
    assert!(matches!(res, Err(crate::error::EngineError::ScriptError(_))));
}
...
async fn edge_complete_task_on_completed_instance() {
    ...
    let res = engine.complete_user_task(uuid::Uuid::new_v4(), std::collections::HashMap::new()).await;
    assert!(matches!(res, Err(crate::error::EngineError::TaskNotPending{..})));
}
...
async fn history_query_limit_and_offset() {
    ...
    let hist_pag1 = engine.persistence.as_ref().unwrap().query_history(crate::persistence::HistoryQuery {
        instance_id: inst_id,
        limit: Some(3),
        offset: Some(0),
        ..Default::default()
    ...
    let hist_pag2 = engine.persistence.as_ref().unwrap().query_history(crate::persistence::HistoryQuery {
        instance_id: inst_id,
        limit: Some(3),
        offset: Some(3),
        ..Default::default()
