...
async fn complete_task_for_node(
    ...
    let task = tasks
        .first()
        .unwrap_or_else(|| panic!("No task found for node {}", node_id));
...
async fn compliance_exclusive_gateway_routing() {
    ...
    let inst_id_high = engine
        .start_instance_with_variables(def_id, HashMap::from([("x".into(), serde_json::json!(15))]))
        .await
        .unwrap();
    let details_high = engine.get_instance_details(inst_id_high).await.unwrap();
    assert_eq!(
        details_high.tokens.values().next().unwrap().current_node,
        "taskA",
        "Compliance Violation: Expected condition x > 10 to route to taskA"
    );

    // Execute with x = 5 (Routes to TaskB via default)
    let inst_id_low = engine
        .start_instance_with_variables(def_id, HashMap::from([("x".into(), serde_json::json!(5))]))
        .await
        .unwrap();
    let details_low = engine.get_instance_details(inst_id_low).await.unwrap();
    assert_eq!(
        details_low.tokens.values().next().unwrap().current_node,
        "taskB",
        "Compliance Violation: Expected default flow to route to taskB when no conditions met"
    );
}
...
async fn compliance_parallel_gateway_sync() {
    ...
    let inst_id = engine
        .start_instance_with_variables(def_id, empty_vars)
        .await
        .unwrap();
    ...
    assert_eq!(
        details.tokens.len(),
        2,
        "Compliance Violation: Parallel split must produce exact number of outgoing tokens"
    );

    let mut current_nodes: Vec<String> = details
        .tokens
        .values()
        .map(|t| t.current_node.clone())
        .collect();
    current_nodes.sort();
    assert_eq!(current_nodes, vec!["taskA", "taskB"]);

    // Complete taskA - token should stop at join gateway and wait for sync
    complete_task_for_node(
        &engine,
        &inst_id,
        "taskA",
    ...
    assert_eq!(
        details2.tokens.len(),
        1,
        "Compliance Violation: Token must wait at joint gateway"
    );
    assert_eq!(
        details2.tokens.values().next().unwrap().current_node,
        "taskB"
    );
...
async fn compliance_complex_gateway_activation() {
    ...
    complete_task_for_node(
        ...
        &inst_id,
        "taskA",
        HashMap::from([("tokens_arrived".into(), serde_json::json!(1))]),
    )
    .await;
    ...
    complete_task_for_node(
        ...
        &inst_id,
        "taskB",
        HashMap::from([("tokens_arrived".into(), serde_json::json!(2))]),
    )
    .await;
    ...
    assert!(
        details2.tokens.values().any(|t| t.current_node == "taskC"),
        "Remaining orphan token should still be at taskC"
    );
    assert!(
        details2
            .audit_log
        ...
        "Complex gateway must have fired early"
    );
}
