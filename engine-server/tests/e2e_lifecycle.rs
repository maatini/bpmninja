...
async fn delete_instance_returns_204() {
    ...
    let res = client.delete(format!("{}/api/instances/{}", base, inst_id))
        .send().await.unwrap();
    assert_eq!(res.status(), 204, "Delete instance should return 204");

    // GET should now 404
    let res = client.get(format!("{}/api/instances/{}", base, inst_id))
        .send().await.unwrap();
...
async fn get_unknown_instance_returns_404() {
    ...
    let res = client.get(format!("{}/api/instances/{}", base, fake_id))
        .send().await.unwrap();
...
async fn delete_definition_without_cascade_with_instances_returns_409() {
    ...
    let res = client.delete(format!("{}/api/definitions/{}", base, def_key))
        .send().await.unwrap();
    ...
    assert!(res.status().is_client_error() || res.status().is_server_error(),
        "Deleting definition with instances should fail: {}", res.status());
}
...
async fn process_timers_returns_count() {
    ...
    let res = client.post(format!("{}/api/timers/process", base))
        .send().await.unwrap();
...
async fn correlate_message_with_no_match_returns_empty() {
    ...
    let res = client.post(format!("{}/api/message", base))
