...
async fn setup_locked_task(topic: &str, xml: &str) -> (String, reqwest::Client, String, String) {
    ...
    // Deploy
    let res = client.post(format!("{}/api/deploy", base))
    ...
    // Start
    let res = client.post(format!("{}/api/start", base))
    ...
    // Fetch & Lock
    let res = client.post(format!("{}/api/service-task/fetchAndLock", base))
...
async fn get_service_tasks_returns_list() {
    ...
    client.post(format!("{}/api/start", base))
        .json(&serde_json::json!({ "definition_key": def_key }))
        .send().await.unwrap();

    // GET /api/service-tasks
    let res = client.get(format!("{}/api/service-tasks", base))
        .send().await.unwrap();
...
async fn fail_service_task_decrements_retries() {
    let (base, client, _inst_id, task_id) = setup_locked_task("test_topic", SERVICE_TASK_BPMN).await;

    let res = client.post(format!("{}/api/service-task/{}/failure", base, task_id))
        .json(&serde_json::json!({
            "workerId": "worker1",
    ...
    let res = client.get(format!("{}/api/service-tasks", base)).send().await.unwrap();
    let tasks: Vec<Value> = res.json().await.unwrap();
    assert_eq!(tasks.len(), 1);
    assert!(tasks[0]["worker_id"].is_null(), "Should be unlocked after failure");
    assert_eq!(tasks[0]["retries"].as_i64().unwrap(), 2, "Retries should be decremented to 2");
}
...
async fn extend_lock_succeeds() {
    ...
    let res = client.post(format!("{}/api/service-task/{}/extendLock", base, task_id))
        .json(&serde_json::json!({
            "workerId": "worker1",
            "newDuration": 120
        }))
...
async fn complete_wrong_worker_returns_conflict() {
    ...
    let res = client.post(format!("{}/api/service-task/{}/complete", base, task_id))
...
async fn bpmn_error_routes_to_boundary_event() {
    let (base, client, inst_id, task_id) = setup_locked_task("err_topic", ERROR_BOUNDARY_BPMN).await;

    let res = client.post(format!("{}/api/service-task/{}/bpmnError", base, task_id))
        .json(&serde_json::json!({
            "workerId": "worker1",
            "errorCode": "MY_ERR"
        }))
    ...
    let res = client.get(format!("{}/api/instances/{}", base, inst_id)).send().await.unwrap();
    let details: Value = res.json().await.unwrap();
    assert_eq!(details["state"], "Completed");
    assert_eq!(details["current_node"], "end_err", "Should reach error end, not normal end");
}
...
async fn complete_nonexistent_task_returns_404() {
    ...
    let res = client.post(format!("{}/api/service-task/{}/complete", base, fake_id))
