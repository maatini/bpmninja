...
async fn start_with_invalid_uuid_format_returns_400() {
    ...
    assert_eq!(res.status(), 400, "Should return 400 Bad Request for invalid UUID");
...
async fn start_with_unknown_definition_uuid_returns_404() {
    ...
    assert_eq!(res.status(), 404, "Should return 404 Not Found for unknown definition");
    
    let body: Value = res.json().await.expect("parse response");
    assert!(body["error"].as_str().unwrap().contains("Definition not found"));
}
...
async fn start_latest_with_unknown_bpmn_id_returns_400() {
    ...
    assert_eq!(res.status(), 400, "Should return 400 Bad Request for unknown latest BPMN id");
    
    let body: Value = res.json().await.expect("parse response");
    assert!(body["error"].as_str().unwrap().contains("No definition found with BPMN ID"));
}
...
async fn start_instance_with_timer_start_returns_400() {
    ...
    assert_eq!(start_res.status(), 400, "Should not be able to normal-start a timer-start process");
    
    let body: Value = start_res.json().await.expect("parse response");
    assert!(body["error"].as_str().unwrap().contains("Use trigger_timer_start"));
}
